...
BEDROCK_RESPONSES_POLL_READ_TIMEOUT_SECONDS = max(2, _env_int("BEDROCK_RESPONSES_POLL_READ_TIMEOUT_SECONDS", 30))
BEDROCK_RESPONSES_POLL_INTERVAL_SECONDS = max(0.2, _env_float("BEDROCK_RESPONSES_POLL_INTERVAL_SECONDS", 2.0))
...
GATEWAY_BREAKER_RESET_SECONDS = max(5, _env_int("GATEWAY_BREAKER_RESET_SECONDS", 60))
DEGRADED_MODE_ENABLED = _env_bool("DEGRADED_MODE_ENABLED", True)
...
SESSION_MEMORY_TTL_SECONDS = max(60, _env_int("SESSION_MEMORY_TTL_SECONDS", 86400))
SESSION_MEMORY_MAX_TURNS = max(1, _env_int("SESSION_MEMORY_MAX_TURNS", 6))
SESSION_MEMORY_MAX_TEXT_CHARS = max(80, _env_int("SESSION_MEMORY_MAX_TEXT_CHARS", 320))
SESSION_MEMORY_REDACT_KEYS = os.getenv(
    "SESSION_MEMORY_REDACT_KEYS",
    "authorization,user_token,auth_token,password,secret,token",
).strip()
...
ORCHESTRATOR_V2_ENABLED = _env_bool("ORCHESTRATOR_V2_ENABLED", True)
POLICY_ENGINE_V2_ENABLED = _env_bool("POLICY_ENGINE_V2_ENABLED", True)
STOCK_AGENT_EXTERNAL_ENABLED = _env_bool("STOCK_AGENT_EXTERNAL_ENABLED", False)

# External Stock Agent integration (MVP external service).
STOCK_AGENT_EXTERNAL_BASE_URL = os.getenv("STOCK_AGENT_EXTERNAL_BASE_URL", "").strip()
STOCK_AGENT_EXTERNAL_ENDPOINT_PATH = (
    os.getenv("STOCK_AGENT_EXTERNAL_ENDPOINT_PATH", "/v1/stock/advisory").strip() or "/v1/stock/advisory"
)
STOCK_AGENT_EXTERNAL_AUTH_TOKEN = os.getenv("STOCK_AGENT_EXTERNAL_AUTH_TOKEN", "").strip()
STOCK_AGENT_EXTERNAL_CONNECT_TIMEOUT_SECONDS = _env_float("STOCK_AGENT_EXTERNAL_CONNECT_TIMEOUT_SECONDS", 2.0)
STOCK_AGENT_EXTERNAL_READ_TIMEOUT_SECONDS = _env_float("STOCK_AGENT_EXTERNAL_READ_TIMEOUT_SECONDS", 8.0)
STOCK_AGENT_EXTERNAL_MAX_RETRIES = max(0, _env_int("STOCK_AGENT_EXTERNAL_MAX_RETRIES", 2))
STOCK_AGENT_EXTERNAL_BACKOFF_FACTOR = max(0.0, _env_float("STOCK_AGENT_EXTERNAL_BACKOFF_FACTOR", 0.4))
STOCK_AGENT_EXTERNAL_BREAKER_FAILURE_THRESHOLD = max(
    1,
    _env_int("STOCK_AGENT_EXTERNAL_BREAKER_FAILURE_THRESHOLD", 3),
)
STOCK_AGENT_EXTERNAL_BREAKER_RESET_SECONDS = max(
    5,
    _env_int("STOCK_AGENT_EXTERNAL_BREAKER_RESET_SECONDS", 60),
)
