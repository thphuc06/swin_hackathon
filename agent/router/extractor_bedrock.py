...
def _build_prompt(user_prompt: str) -> str:
    return (
        ...
        "Educational investing questions are in scope. Questions about stocks, ETFs, bonds, crypto, market overviews, diversification, portfolio considerations, and what to watch before buying should be classified as invest.\n"
        "If prompt is not about personal finance advisory or education-only investing guidance (cashflow, budgeting, non-investment risk, planning, what-if, or educational investing), classify as out_of_scope.\n"
        ...
        "CRITICAL ROUTING INSTRUCTION: You must semantically evaluate the prompt and assign `target_agent_id` to one of the following:\n"
        "- 'service': If the user explicitly asks for a step-by-step roadmap, actionable plan, milestones, or how to reach a savings/purchase goal (e.g., 'roadmap', 'lộ trình', 'mua nhà', 'cách tiết kiệm').\n"
        "- 'planner': If the user asks for current financial analysis, budget tracking, cashflow summaries, cashflow scenarios (what-ifs), or anomaly detection.\n"
        "- 'stock': If the user asks about investments, stocks, crypto, portfolio, or market tone.\n"
        "- null: If the request is out_of_scope or too ambiguous.\n"
        "Output JSON fields: schema_version, intent, target_agent_id, sub_intent, confidence, domain_relevance, top2, slots, scenario_confidence, reason.\n"
